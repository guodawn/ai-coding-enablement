...
1. 一条结论应该相信哪种事实源？
2. 文档如何按 owner 组织，而不是按阅读顺序堆放？
3. 人与 Agent 如何共享正文，并按 L0–L3 渐进加载？

> 这一章的产出是“责任地图”：知道该到哪里找事实，也知道不该把什么混在一起。

### 讲者备注

docs-for-ai 的重点不是多写文档，而是消除事实冲突时的模糊性。后续每一页都会回到一个原则：判断归属、修正 owner、让派生物可验证。

### 过渡

先从最关键的判断开始：这条结论究竟属于哪类事实？
...
- **Route：** 以 lane、domain、condition 确定最小行动包；
- **Execute / Review：** 在代码中寻找根因，取得并解释证据。

Route 约束入口、边界与证据责任；它**不**替 Agent 判断根因，也不替人做高影响取舍。

### 讲者备注

这一章最容易误解 Route 是“自动解决问题”。它的价值是相同分类的任务不再因不同 Agent 而遗漏不同规则、文档或验证；根因仍由当前代码与运行证据决定。

### 过渡

先看 Route 的输入、输出和明确边界。
...
## 12. Lane、Domain、Condition：三个互补维度
...
1. 用户错误被改写为带边界的工程任务；
2. 第一层根因定位到跨域请求缺少执行人身份；
3. focused 证据通过后，真实验收仍推翻第一次 Close；
4. 第二层根因进入回归测试、DEFECT 与 Delivery Log。

> 目标不是“一次猜中”，而是让新证据能安全地改变结论。

### 讲者备注

这不是一个“AI 一次修好”的故事。它说明为什么高质量交付需要分层验证和可恢复的 Explore：局部机制成立，仍可能无法证明真实用户路径。
