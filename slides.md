...
## 20. 第三章总览：两轮定位，才完成一次故障交付

### 屏幕内容

**这不是一次修好的故事，而是一条被运行证据两次推动的交付链。**

```text
用户报错：Dataset 节点失败
  ↓
第一轮：发现跨服务请求漏带执行人身份
  → 补齐 4 个调用点，focused tests 通过
  ↓
真实 test_run 仍失败：knowledge identity is required
  ↓
第二轮：发现 cache reset 遮蔽了 request session
  → 红测定义边界，修复后完成真实验收
```

**最后才 Close：把两层根因、回归测试和验收证据分别写回正确的 owner。**

### 讲者备注

先给观众一条能记住的主线：第一次修复并不是错误，而是只证明了局部机制；真实复测暴露了第二层问题。案例的价值在于，新证据让任务重新回到 Explore，而不是把第一次结论硬撑到 Close。
